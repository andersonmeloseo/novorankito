...
const severityConfig = {
  ...
  high: { icon: TrendingUp, color: "bg-warning/10 text-warning border-warning/20", label: "High" },
  medium: { icon: Info, color: "bg-info/10 text-info border-info/20", label: "Medium" },
  low: { icon: Zap, color: "bg-muted text-muted-foreground border-border", label: "Low" },
};
...
export function InsightCard({ severity, title, description, impact, action }: InsightCardProps) {
  ...
  return (
    <Card className={cn("p-4 border", config.color.split(" ").pop())}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          <div className="flex items-center justify-between gap-2 pt-1">
            <span className="text-xs font-medium text-foreground">Impact: {impact}</span>

...
serve(async (req) => {
  ...
  try {
    const { project_id, query, days = 90 } = await req.json();
    if (!project_id || !query) {
      return new Response(JSON.stringify({ error: "project_id and query are required" }), {
    ...
    if (connErr || !conn || !conn.site_url) {
      return new Response(JSON.stringify({
        ...
        rows: [],
        noConnection: true,
        message: "GSC connection not found",
      }), {
        status: 200,
    ...
    startDate.setDate(startDate.getDate() - Math.min(days, 480));

    const fmt = (d: Date) => d.toISOString().split("T")[0];

    // Fetch date + query data for the specific keyword
    const res = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        dimensionFilterGroups: [{
          filters: [{ dimension: "query", expression: query, operator: "equals" }],
        }],
        rowLimit: 25000,
      }),
    });
    ...
    console.error("gsc-position-history error:", error);

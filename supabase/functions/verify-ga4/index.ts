...
serve(async (req) => {
  ...
  try {
    ...
    if (body.project_id) {
      ...
      const { data: conn } = await sb.from("ga4_connections").select("client_email, private_key").eq("project_id", body.project_id).maybeSingle();
      if (!conn) {
        return errorResponse("Nenhuma conexão GA4 encontrada para este projeto.", cors, 404);
      }
    ...
    const accessToken = await getGoogleAccessToken({ client_email: clientEmail, private_key: privateKey }, "https://www.googleapis.com/auth/analytics.readonly");
